...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        mainPanel(
           fillPage(plotOutput("yearplot")) # fillPage helped it fit on the markdown output
        )
    )
)
...
server <- function(input, output) {
    ...
    output$yearplot <- renderPlot({
       ggplot(data = (df %>% 
            ...
            geom_line(size = 1) + geom_point() + labs(color='Year') +
...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        mainPanel(
            fillPage(plotOutput("winterPlot"))
        )
    )
)
...
It is clear from this graph that the 2020/2021 winter period does not follow the same trend as previous years, with cases remaining very low.
